...
#define FILENAME L"CDXWindow12.cpp"
...
HRESULT CDXWindow12::Initialize(const DXWINDOW_DESC& Desc, IUnknown* pDevice, IDXWindowCallback* pDXWindowCallback) {
...
VOID CDXWindow12::Present(UINT SyncInterval, UINT Flags) {
...
LRESULT CDXWindow12::WindowProcess(UINT Message, WPARAM wParam, LPARAM lParam) {
	HRESULT hr = S_OK;
...
VOID CDXWindow12::UpdateWindowState() {
	BOOL bresult = TRUE;
...
VOID CDXWindow12::ToggleFullscreen() {
	HRESULT hr = S_OK;
...
VOID CDXWindow12::FullscreenWindow() {
	HRESULT hr = S_OK;
	BOOL bresult = TRUE;
	RECT DesktopArea;
...
HRESULT CDXWindow12::RegisterWindowClass(const DXWINDOW_DESC& Desc) {
	HRESULT hr = S_OK;
	DWORD err = ERROR_SUCCESS;
	...
	wc.lpfnWndProc = WindowMessageDispatcher12::StaticWindowProcess;
...
HRESULT CDXWindow12::MakeWindow(const DXWINDOW_DESC& Desc) {
	HRESULT hr = S_OK;
	DWORD err = ERROR_SUCCESS;

	Output12* output = m_OutputEnum.PrimaryOutput();
	RECT WindowArea;
...
VOID CDXWindow12::SetFocus() {
	m_InFocus = TRUE;
}

VOID CDXWindow12::CenterCursor() {
	BOOL bresult = TRUE;
	HRESULT hr = S_OK;
...
VOID CDXWindow12::SetState(DXWINDOW_STATE State) {
	if (State != m_State) {
		if (State == DXWINDOW_STATE_FULLSCREEN) {
			m_Callback->OnBackBufferRelease(this);
...
VOID CDXWindow12::SetWindowResolution(WORD Width, WORD Height) {
